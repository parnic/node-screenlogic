...
async function app() {
  ...
  const password = '1111';
  let unit: SLGateWayData;
  try {
    const finder = new FindUnits();
    const localUnits: LocalUnit[] = await finder.searchAsync();
    finder.close();
    ...
    else console.log('No local units found');
  }
  catch (err) {
  ...
  */
  client.on('intellibriteDelay', function (sec: number) {
  ...
  try {
    ...
    dt.setHours(12);
    const sysTime = await screenlogic.equipment.setSystemTimeAsync(dt, true);
    console.log(`set time result: ${sysTime}`);
    const hist = await screenlogic.equipment.getHistoryDataAsync();
    ...
    const chemHist = await screenlogic.chem.getChemHistoryDataAsync();
