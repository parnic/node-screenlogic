...
async function app() {
  ...
  const systemName = 'Pentair: 00-45-03';
  const password = '1111';
...
/*   finder.on('serverFound', function(server) {
  ...
  client.on('loggedIn', function () {
    ...
    console.log(' pool temp=' + status.bodies[0].currentTemp);
  ...
  try {
    ...
    // let pumpStatus2 = await client.pump.getPumpStatus(1);
    // console.log(`Pump 1: ${JSON.stringify(pumpStatus2)}`);
   ...
   setTimeout(async () => {
     console.log(`closing connection after 120s`);
